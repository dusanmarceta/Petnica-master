...
def run_command(command):
    subprocess.run(f'bash -c "{command}"', shell=True, check=True, stderr=subprocess.STDOUT, preexec_fn=os.setpgrp)
...
def cart2orb(x, y, z, vx, vy, vz, G):
    ...
    if np.dot(r, v) < 0 and np.linalg.norm(e) < 1.:
        nu = 2 * np.pi - nu
    elif np.dot(r, v) < 0 and np.linalg.norm(e) > 1.:
        nu = -nu
    ...
    # argument pericentra
    o = np.arccos(np.dot(n, e) / np.linalg.norm(n) / np.linalg.norm(e))
    if e[2] < 0:
        o = 2 * np.pi - o

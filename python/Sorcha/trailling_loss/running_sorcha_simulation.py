...
for slope in slope_values:
    ...
    for i in range(1, 21):
        ...
        output_file = '../results/trailling/A_M_' + slope_formated + '_1.0_output_file_' + str(i)
        
#        print('aaaaa')
#        command = f'nohup sorcha run -c css_configuration.ini -p {pp_file} -ob {orbit_file} -pd {cadence_file} -o ./ -t {output_file} > /dev/null 2>&1 &'
        

        command = f"nohup sorcha run -f -c ../Rubin_circular_approximation.ini -p {pp_file} -ob {orbit_file} -pd {cadence_file} -o ./ -t {output_file} > log_file.log 2>&1 &"

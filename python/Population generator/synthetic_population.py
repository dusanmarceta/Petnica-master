...
#        x=np.linspace(N_ref[0], N_ref[-1], total_number) # uniform sample which is transformed using the Inverse Transform Sampling method
        ...
        x = np.random.random(total_number) * (N_ref[-1] - N_ref[0]) + N_ref[0] # uniform sample which is transformed using the Inverse Transform Sampling method
        D_s=np.zeros_like(x)
...
def total_number(rm, n0, v_min, v_max,
                         ...
                         d_ref=1000, d=[], alpha=[], maximum_array_size=int(1e7)):
...
def speed_distribution(v_min, v_max, u_Sun, v_Sun, w_Sun, sigma_vx, sigma_vy, sigma_vz, vd, va, speed_resolution_out=10, speed_resolution_in=100, angle_resolution=90, maximum_array_size=int(1e7)):
    '''
...
def synthetic_population(rm, n0, v_min, v_max,
                         ...
                         vd, va, R_reff,
                         speed_resolution=100, angle_resolution=90, dr=0.1,
                         d_ref=1000, d=[], alpha=[], maximum_array_size=int(1e8)):

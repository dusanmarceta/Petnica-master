...
albedo = np.linspace(0.05, 0.4, 50)

D, albedo = np.meshgrid(D, albedo)

H = 15.618 - 2.5 * np.log10(albedo) - 5 * np.log10(D / 1000)

C = 10 ** ((V_cut - H) / 5)
distance = (1 + np.sqrt(1 + 4 * C)) / 2

plt.figure()
contour = plt.contourf(D/1000, albedo, distance, 100, cmap='jet')
plt.xlabel('D (km)', fontsize=40)
plt.ylabel('albedo', fontsize=40)

plt.xticks(fontsize=36)
plt.yticks(fontsize=36)

cbar = plt.colorbar(contour)
cbar.set_label('Detection Limits of LSST (au)', fontsize=40)
